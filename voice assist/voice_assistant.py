...
def takeCommand():
    ...
    with sr.Microphone() as source:
        ...
        # User requested a "Listening" popup like Google
        try:
            popup = tkinter.Tk()
            popup.title("Jarvis")
            popup.overrideredirect(True) # Remove title bar for cleaner look
            ...
            y = (screen_height - height - 100) # Bottom center looks more like an assistant
            popup.geometry(f'{width}x{height}+{x}+{y}')
            
            # Add content
            popup.configure(bg='#202124') # Dark Google-ish background
            label = tkinter.Label(popup, text="🎤 Listening...", font=("Segoe UI", 16), fg="white", bg='#202124')
            label.pack(expand=True)
            
            popup.update() # Force drawing before blocking listen
        except Exception as e:
            print("Popup Error:", e)
            popup = None
        ...
        r.pause_threshold = 1
        try:
            # We must destroy the popup after listening, even if recognition fails
